# AZ-900 Azure Fundamentals – Focused Study Plan

This repository documents a **time-efficient, exam-focused study plan** for the  
**AZ-900: Microsoft Azure Fundamentals** certification.

It combines:
- ✅ **John Savill’s AZ-900 video series** (best signal-to-noise)
- ✅ **Official Microsoft instructor-led AZ-900 videos** (official wording & framing)

Target audience:
- Engineers, developers, analysts, architects
- Strong technical background
- Goal: **pass confidently**, not over-study

---

## 🧠 Study Strategy

- **Primary source:** John Savill (exam-focused explanations)
- **Secondary source:** Microsoft official videos (terminology alignment)
- Focus on:
  - Definitions
  - Use cases
  - Comparisons
- Avoid:
  - Deep configuration detail
  - SKU-level or pricing minutiae

Time commitment:
- Weekdays: **60–75 mins**
- Weekends: **~90 mins**

---

## 📅 Day-by-Day Study Plan

---

## 📅 Day 1 – Cloud Concepts (High Yield)

### John Savill
- [x] CapEx, OpEx & Consumption-based  
  https://youtu.be/WiwV9wb0GMo
- [x] Differences Between Cloud Service Categories  
  https://youtu.be/IqQC1EOQqeU
- [x] Identify the Right Service Type  
  https://youtu.be/KH8NH76h2vc
- [x] Differences Between Types of Cloud Computing  
  https://youtu.be/7dlCrF2wmXU
- [x] Benefits of High Availability & Scalability  
  https://youtu.be/JRbhGzGzoOA

### Microsoft
- [ ] AZ-900 Module 1 – Cloud Fundamentals  
  https://learn.microsoft.com/en-us/shows/on-demand-instructor-led-training-series/az-900-module-1

Focus:
- [ ] Shared responsibility model
- [ ] Deployment models
- [ ] Consumption-based pricing

---

## 📅 Day 2 – Reliability & Azure Global Architecture

### John Savill
- [x] Reliability and Predictability  
  https://youtu.be/kD2YqdDaO1w
- [x] Benefits & Usage of Regions and Region Pairs  
  https://youtu.be/4RjPOAN54AE
- [x] Benefits & Usage of Availability Zones  
  https://youtu.be/h0enGb17lnw

### Microsoft
- [ ] AZ-900 Module 2 – Azure Architecture & Compute  
  https://learn.microsoft.com/en-us/shows/on-demand-instructor-led-training-series/az-900-module-2

Key takeaways:
- [ ] Availability Zones ≠ Regions
- [ ] Region pairs = disaster recovery

---

## 📅 Day 3 – Azure Hierarchy & Control Plane

### John Savill
- [x] Resource Groups  
  https://youtu.be/g6thrYZhPZY
- [x] Subscriptions  
  https://youtu.be/9vKAYW_WkLo
- [x] Management Groups  
  https://youtu.be/bPdDiEtCVhM
- [x] Azure Resource Manager (ARM)  
  https://youtu.be/g4u0NL2-3XM
- [x] Azure Arc (optional)  
  https://youtu.be/cW6_rvDYSHg

Memorise:
- [ ] Management Group > Subscription > Resource Group > Resource

---

## 📅 Day 4 – Compute Services

### John Savill
- [x] Resources Required for Virtual Machines  
  https://youtu.be/PP5BWZ0cAJo
- [x] Core Compute Resources (VMs, Scale Sets, Containers, Functions)  
  https://youtu.be/yKDSAYDLGrI

Optional:
- [ ] Azure Dedicated Hosts  
  https://youtu.be/RnNqmTH9xok

Know:
- [ ] VM vs Container vs Function
- [ ] Scale Sets = auto-scaling VMs
- [ ] Functions = serverless

---

## 📅 Day 5 – Networking & Storage (Exam Favourites)

### John Savill
- [x] Core Network Resources  
  https://youtu.be/aNK0C9Oj2sg
- [x] Public and Private Endpoints  
  https://youtu.be/bPNkXwRFsek
- [x] Storage Account Resources  
  https://youtu.be/b8BrfsxLSx8

### Microsoft
- [ ] AZ-900 Module 3 – Azure Networking  
  https://learn.microsoft.com/en-us/shows/on-demand-instructor-led-training-series/az-900-module-3
- [ ] AZ-900 Module 4 – Azure Storage  
  https://learn.microsoft.com/en-us/shows/on-demand-instructor-led-training-series/az-900-module-4

Exam traps:
- [ ] VPN Gateway ≠ ExpressRoute
- [ ] Blob ≠ File
- [ ] Archive tier = cheap, slow

---

## 📅 Day 6 – Migration, Marketplace & Serverless

### John Savill
- [ ] Data Movement and Migration Options  
  https://youtu.be/jNBcXnMTo9s
- [ ] Azure Marketplace  
  https://youtu.be/b7RuB4Bymgc
- [ ] Serverless Technologies  
  https://youtu.be/-xeJGiMw5OE

Know:
- [ ] Azure Migrate = assessment + migration
- [ ] Data Box = offline data transfer

---

## 📅 Day 7 – Security Fundamentals

### John Savill
- [ ] Microsoft Defender for Cloud  
  https://youtu.be/eWcoMi_nQt4
- [ ] Defense in Depth  
  https://youtu.be/CHKS2FcEMek
- [ ] Zero Trust  
  https://youtu.be/JX3w4to-qgo
- [ ] Network Security Groups (NSGs)  
  https://youtu.be/flCoRc1uv9o
- [ ] Azure Firewall  
  https://youtu.be/1-_cqpUISg4
- [ ] Azure DDoS Protection  
  https://youtu.be/6cv-wdo9TJw

Optional:
- [ ] Key Vault  
  https://youtu.be/ZBXVAD4S0Tc
- [ ] Microsoft Sentinel  
  https://youtu.be/xaqiPXL6tz0

---

## 📅 Day 8 – Identity, Governance & Compliance

### John Savill
- [ ] Authentication vs Authorization  
  https://youtu.be/GA-yNu6aFMk
- [ ] Azure Directory Services  
  https://youtu.be/E4__JBVE25I
- [ ] External Identities  
  https://youtu.be/G5_z4PFgn2o
- [ ] Conditional Access, MFA & SSO  
  https://youtu.be/DFwERh9Xxk0
- [ ] Overview of Microsoft Entra  
  https://youtu.be/bSIF_GjaCmo
- [ ] RBAC  
  https://youtu.be/0iVyJBG06fM
- [ ] Resource Locks  
  https://youtu.be/eF_KilJRxbE
- [ ] Tags  
  https://youtu.be/eaf63hE_6SQ
- [ ] Azure Policy  
  https://youtu.be/z7WMqHE3R8g

Optional / skim:
- [ ] Microsoft Purview  
  https://youtu.be/mXjXcBr1ajY
- [ ] Azure Blueprints  
  https://youtu.be/SJbDcvkySCY
- [ ] Cloud Adoption Framework  
  https://youtu.be/VHQxbIynlN4

---

## 📅 Day 9 – Cost Management & SLAs

### John Savill
- [ ] Factors That Affect Costs  
  https://youtu.be/fMShW_RGcxY
- [ ] Factors to Reduce Cost  
  https://youtu.be/B5yiKE2DLH8
- [ ] Pricing & TCO Calculators  
  https://youtu.be/pE-bf8i5blU
- [ ] Azure Cost Management  
  https://youtu.be/FoBjC9CAF08
- [ ] Service Level Agreements (SLAs)  
  https://youtu.be/3QIVbgnNrR0
- [ ] Azure Service Lifecycle  
  https://youtu.be/NJ9-S7C0pY4

Know cold:
- [ ] SLA = uptime guarantee
- [ ] SLA ≠ performance guarantee
- [ ] Composite SLAs multiply

---

## 📅 Day 10 – Final Review (Day Before Exam)

- [ ] No new videos
- [ ] Review IaaS / PaaS / SaaS
- [ ] Review shared responsibility model
- [ ] Review region vs availability zone
- [ ] Review RBAC vs Conditional Access
- [ ] Review serverless definition
- [ ] Light skim of John Savill handout
- [ ] Early night 😴

---

## 🧪 Exam Technique Tips

- [ ] Read every word of each question
- [ ] Look for:
  - “Best”
  - “Least administrative effort”
  - “Most cost-effective”
- [ ] When unsure, choose the **simplest, most managed** service

---

## 🎯 Outcome

Following this plan should result in:
- Clear conceptual understanding
- Familiarity with Microsoft’s terminology
- Comfortable pass margin

AZ-900 rewards **clarity, not depth**.
